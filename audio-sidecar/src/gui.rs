...
impl UI {
    ...
    pub fn button(&self, gfx: &Gfx, text: &str, x: f32, y: f32, width: f32, height: f32) -> bool {
...
pub fn draw_waveform(gfx: &Gfx, waveform: &[u32], x: f32, y: f32, width: f32, height: f32) {
        ...
        .enumerate()
    {
        ...
        if *m >= MAX_AMPLITUDE - 1 {
            ...
            or_die(gfx.render_line(x + col as f32, y, x + col as f32, y + height));
            or_die(gfx.set_render_draw_color(255, 255, 255, 255));
        } else {
...
pub fn debug_view(gfx: &Gfx, text: &str) {
    const GLYPH_SIZE: f32 = 8.0;
    ...
    for line in text.lines() {
        gfx.render_debug_text(line, 0.0, y).unwrap();
        y += LINE_SIZE;
    }
}
